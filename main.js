...
const commands = [
    new SlashCommandBuilder()
        .setName('vouch')
        .setDescription('Gửi đánh giá cho một người')
        .addStringOption(option =>
            option.setName('san_pham')
                .setDescription('Tên sản phẩm khách mua')
                .setRequired(true))
        .addIntegerOption(option =>
            option.setName('diem')
                .setDescription('Điểm đánh giá (1-5)')
                .setRequired(true)
                .setMinValue(1)
                .setMaxValue(5))
        .addStringOption(option =>
            option.setName('noi_dung')
                .setDescription('Nội dung đánh giá')
                .setRequired(true))
        .addUserOption(option =>
            option.setName('cho')
                .setDescription('Người bạn muốn đánh giá')
                .setRequired(true))
        .addAttachmentOption(option =>
            option.setName('anh')
                .setDescription('Ảnh minh chứng (tùy chọn)')
                .setRequired(false))
        .addStringOption(option =>
            option.setName('link_anh')
                .setDescription('Link ảnh minh chứng (tùy chọn)')
                .setRequired(false)),
...
client.on('interactionCreate', async (interaction) => {
        ...
        switch (commandName) {
            case 'vouch':
                ...
                if (!isVouchChannelAllowed(interaction.channel.id)) {
                    ...
                    const sanPham = interaction.options.getString('san_pham');
    const rating = interaction.options.getInteger('diem');
    const message = interaction.options.getString('noi_dung');
    const targetUser = interaction.options.getUser('cho');
    const imageAttachment = interaction.options.getAttachment('anh');
    const imageUrl = interaction.options.getString('link_anh');
                ...
                // Embed gửi lên kênh review
    const reviewEmbed = new EmbedBuilder()
        .setDescription(
            `**Sản phẩm** : ${sanPham}\n` +
            `**Khách Hàng đã mua** : ${targetUser}\n` +
            `**Đánh Giá** : ${generateRatingStars(rating)}\n` +
            `**Feedback** : ${message}\n` +
            `**Proof** : ${vouch.imageUrl ? '[Image Link](' + vouch.imageUrl + ')' : 'Không có'}`
        )
        .setColor(config.embedColor || 0x000000)
    ...
    if (vouch.imageUrl) {
        reviewEmbed.setImage(vouch.imageUrl);
    }

    const reviewChannel = client.channels.cache.get(config.reviewChannelId);
    if (reviewChannel) {
        reviewChannel.send({ embeds: [reviewEmbed] });
    }

    await interaction.reply({ content: '✅ Cảm ơn bạn đã gửi đánh giá!', ephemeral: true });
                break;
